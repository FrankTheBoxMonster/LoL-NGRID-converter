...
The Season 10 map changes introduced the concept of .ngrid_overlay files, which simply provide a set of new vision/pathing flags to apply to all of the cells on the map.  An overlay exists for Infernal, Mountain, and Ocean Drakes, but not for Cloud Drake, which instead exists on the POI flag layer, which no longer features POI regions for each jungle camp (although retains one for "near turrets" despite the removal of the Raptor Cloak items and Point Runner passive because it is still used for Heimerdinger's passive).

Additionally, a new flag layer was added, but its significance is unknown.  It roughly appears to be a copy/paste of vision/pathing flags but with structure walls removed, and additionally features new distinct regions around some of the jungle camps, while also making a distinction between top side and bot side variations of these new areas.  These areas roughly correspond to puddles that are created when Ocean Drake takes over the map, however the area between blue buff and wolves noticeably lacks a puddle in-game.  These areas are also where the additional Honeyfruit will spawn, however, a region marker would not influence these plant spawns, and no other plants require region markers.  It is possible that these areas once served as an AOE healing zone (like mini fountains, because ocean --> water --> fountain --> etc.) but were at some point replaced with Honeyfruit, which serve the same purpose but may have had less game-warping effects (there may have been problems with people trying to camp the mini fountains, but Honeyfruit spawn timers would make the areas not *always* valuable and worth defending as well as giving more interactive gameplay against poke or in teamfights than what would be achieved with something like "mini fountains are disabled while in combat").