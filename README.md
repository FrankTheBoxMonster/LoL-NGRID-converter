...
[Download links](https://github.com/FrankTheBoxMonster/LoL-NGRID-converter/releases/tag/v1.2)
...
![Overlay_Infernal.bmp](https://raw.githubusercontent.com/FrankTheBoxMonster/LoL-NGRID-converter/master/test%20files/SummonersRiftSeason10/SummonersRiftSeason10.VisionPathing.Overlay_Infernal.bmp)

![Cloud_POI.bmp](https://raw.githubusercontent.com/FrankTheBoxMonster/LoL-NGRID-converter/master/test%20files/SummonersRiftSeason10/SummonersRiftSeason10.POI.bmp)

![SRX.bmp](https://raw.githubusercontent.com/FrankTheBoxMonster/LoL-NGRID-converter/master/test%20files/SummonersRiftSeason10/SummonersRiftSeason10.SRX.bmp)

The Season 10 map changes introduced the concept of .ngrid_overlay files, which simply provide a set of new vision/pathing flags to apply to all of the cells on the map.  Overlays are only used for Infernal, Mountain, and Ocean Drakes.  Cloud Drake does not use an overlay, and instead exists on the POI flag layer, which no longer features POI regions for each jungle camp (although retains the regions for "near turrets" despite the removal of the Raptor Cloak items and Point Runner passive because these turret regions are still used for Heimerdinger's passive).

Additionally, a new flag layer was added, but its significance is unknown.  It roughly appears to be a copy/paste of vision/pathing flags but with structure walls removed, and additionally features newly-defined regions around some of the jungle camps, while also making a distinction between top side and bot side variations of these new areas.  These areas roughly correspond to puddles that are created when Ocean Drake takes over the map, however the area between blue buff and wolves noticeably lacks a puddle in-game.  These areas are also where the additional Honeyfruit will spawn.  However, these additional plant spawns would not require a new regional marker to work, and no other plant spawns require region markers, including normal Honeyfruit.  It is possible that these areas once served as an AOE healing zone (like mini fountains, because ocean --> water --> fountain etc.) but were at some point replaced with Honeyfruit, which serve the same purpose of providing out-of-base sustain, but may have had less game-warping effects (there may have been problems with people trying to camp the mini fountains, but Honeyfruit spawn timers would make these areas not *always* valuable and worth defending, as well as giving more interactive gameplay against poke or in teamfights than what would be achieved with something like "mini fountains are disabled while in combat").